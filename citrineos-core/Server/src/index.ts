...
export class CitrineOSServer {
  ...
  constructor(
    ...
    /* c8 ignore start */
    const tempDir = process.env.NODE_V8_COVERAGE || '/usr/local/apps/citrineos/coverage/tmp';
    const COVERAGE_DIR = '/usr/local/apps/citrineos/coverage';
    const FINAL_PATH = path.join(COVERAGE_DIR, 'coverage-final.json');
    let mergedMap = createCoverageMap({});
    ...
    const query = parsedUrl.query;
        if (pathname === '/v8dump') {
          v8.takeCoverage();
            exec('npm run report-c8', (err: Error | null, stdout: string, stderr: string) => {
            if (err) {
              console.error('Error running c8 report:', err);
              res.writeHead(500);
              res.end('Failed to run report');
              return;
            }

            console.log('c8 report, stdout:', stdout);

            if (fs.existsSync(FINAL_PATH)) {
              const latest = JSON.parse(fs.readFileSync(FINAL_PATH, 'utf-8'));
              const latestMap = createCoverageMap(latest);

              mergedMap.merge(latestMap);
              const mergedJson = mergedMap.toJSON();

              if (query.init === '1'){
                console.log('init == 1!!');
                for (const file of Object.values(mergedJson)) {
                  for (const key in file.s) file.s[key] = 0;
                  for (const key in file.f) file.f[key] = 0;
                  for (const key in file.b) file.b[key] = file.b[key].map(() => 0);
                }

                mergedMap = createCoverageMap(mergedJson);
                console.log(JSON.stringify(mergedMap.toJSON(), null, 2));
              }
              fs.writeFileSync(FINAL_PATH, JSON.stringify(mergedJson, null, 2));
              console.log('✅ coverage-final merged');

              const reportDir = path.join(COVERAGE_DIR, 'report');
              const cacheDir = path.join(reportDir, '.cache');

              const report = new CoverageReport({
                outputDir: path.join(COVERAGE_DIR, 'report'),
                reports: ['html', 'json-summary']
              });
              const summaryEntry = path.join(path.join(COVERAGE_DIR, 'report'), 'coverage-summary.json');
              let beforeMtime = 0;
              if (fs.existsSync(summaryEntry)) {
                beforeMtime = fs.statSync(summaryEntry).mtimeMs;
              }
              ...
              let updated = false;
              (async () => {
                report.add(mergedMap.toJSON());
              async function generateWithTimeout(retry = 0) {
                ...
                try {
                  await Promise.race([
                    report.generate(),
                    new Promise((_, reject) => setTimeout(() => reject(new Error('Monocart timeout')), TIMEOUT_MS))
                  ]);
                  console.log(`success`);
                } catch (err) {
                  console.error(`fail`);
                  if (retry < MAX_RETRY) {
                    console.log(`Monocart retry... (${retry + 2}/${MAX_RETRY + 1})`);
                    await generateWithTimeout(retry + 1);
                  } else {
                    console.error(`retry fail`);
                  }
                }
              }
              if (fs.existsSync(summaryEntry)) {
                fs.unlinkSync(summaryEntry);
                console.log('coverage-summary.json delete');
              }
              ...
              const tmpDir = '/usr/local/apps/citrineos/coverage/tmp';
              fs.readdirSync(tmpDir)
                .filter(f => f.endsWith('.json'))
                .forEach(f => fs.unlinkSync(path.join(tmpDir, f)));
              if (updated){
                res.writeHead(200, { 'Content-Type': 'text/plain' });
                res.end('Coverage updated and monocart report generated');
              }else{
                res.writeHead(500);
                res.end('coverage-final.json not found');
              }
              })();
            } else {
              res.writeHead(500);
  ...
  console.log('[child] v8dump: http://localhost:9999/v8dump');
});
    /* c8 ignore stop */
