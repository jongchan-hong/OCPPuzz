from storage.entity.base_entity import BaseEntity
from sqlalchemy import Column, String
from sqlalchemy.orm import relationship
from storage.entity.rule_collect_detail_entity import RuleCollectDetailEntity
from storage.entity.rule_collect_fail_entity import RuleCollectFailEntity
from storage.entity.gpt_rule_collect_log_entity import GPTRuleCollectLogEntity
from util.hash import calculate_sha256


class RuleCollectEntity(BaseEntity):
    __tablename__ = 'rule_collect'

    file_path = Column(String(255), nullable=False)
    check_sum_hash = Column(String(255), nullable=False, default="")
    detail_list = relationship(RuleCollectDetailEntity, back_populates="rule_collect_entity",
                               cascade="all, delete-orphan")
    fail_list = relationship(RuleCollectFailEntity, back_populates="rule_collect_entity",
                             cascade="all, delete-orphan")
    log_list = relationship(GPTRuleCollectLogEntity, back_populates="rule_collect_entity",
                            cascade="all, delete-orphan")

    generates = relationship("GenerateRuleCollectEntity", back_populates="rule_collect", cascade="all, delete-orphan")

    def __init__(self, file_path: str):
        super().__init__()
        self.file_path = file_path
        self.check_sum_hash = calculate_sha256(file_path)